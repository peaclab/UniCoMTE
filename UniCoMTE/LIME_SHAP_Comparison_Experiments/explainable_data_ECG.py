...
class ClfData:
    ...
    def wrap_df_x(x_train, feature_size):
        ...
        return x_train_df
    @staticmethod
    def wrap_df_x_ts_fresh(x_train, feature_size):
        ...
        #reshape data for easier conversion to DF. flatten last dimension into columns (attributes)
        #reshaped_data = timeseries.reshape((-1, timeseries.shape[-1]))
        print(f"timeseries shape is {timeseries.shape}")

        # Step 1: Reshape the numpy array into a pandas DataFrame
        # Create a MultiIndex from the sample IDs and timestamps
        sample_ids = np.arange(20000)  # 20000 unique samples
        timestamps = np.arange(4096)   # 4096 timestamps

        # Create the DataFrame
        df = pd.DataFrame(
            timeseries.reshape(-1, 12),  # Flatten the data: 20000 * 4096 rows, 12 columns
            columns=[f"measurement_{i}" for i in range(12)]
        )

        # Create MultiIndex for the DataFrame
        df['sample_id'] = np.repeat(sample_ids, 4096)  # Repeat sample_ids 4096 times
        df['timestamp'] = np.tile(timestamps, 20000)  # Repeat timestamps 20000 times

        return df
    
    def wrap_df_x_SHAP(x_train, feature_size):
        ...
        print(f'Cutting out the first {num_features} features')

        #reshape data for easier conversion to DF. flatten last dimension into columns (attributes)
        #reshaped_data = timeseries.reshape((-1, timeseries.shape[-1]))
        print(f"timeseries shape is {timeseries.shape}")

        df = timeseries[:num_features,:,:]
        
        return df
    
    def wrap_df_x_LIME(x_train, feature_size):

        '''
        input: np array (N,4096,12) 
        output: np array (n_features, 4096*12)
        '''
        feature_size = 12
        num_features = 100
        ...
        df = timeseries[:num_features,:,:]
        df_flattened = df.reshape(num_features, 4096 * feature_size)  # Shape: (num_features, 49152)
        print(f"Final shape after flattening: {df_flattened.shape}")  # Should be (num_features, 49152)
    ...
    def wrap_df_y(y_train):
        ...
        if not onehot_labels:
            ...
            # Find rows with all False (no '1') # rows with all false become true
    ...
    def wrap_df_y_ts_fresh(y_train):
        """
        Create the specific pandas dataframe sample label needed to pass through explainer.
        ...
        y_train_df_np = y_train_df.values.ravel()


        return y_train_df_np.astype(int)
    ...
    def wrap_df_test_point_SHAP(test_point):
        """
        Wraps up a single test point into an array and then wrap that into a dataframe
        so that it can pass through explainer.
        ...
        """
        # convert a 3D np dataframe to a 2D pd dataframe
        print(f'sample shape for SHAP is {test_point.shape}')
        
        
        return test_point

    def wrap_df_test_point_LIME(test_point):
        """
        Wraps up a single test point into an array and then wrap that into a dataframe
        so that it can pass through explainer.
        ...
        # convert a 3D np dataframe to a 2D pd dataframe
        print(f'initial sample shape for lime is {test_point.shape}')
        
        feature_size = 12
        test_point_reshaped = test_point.reshape(1, 4096 * feature_size)

        print(f'final is: {test_point_reshaped.shape}')

        return test_point_reshaped
