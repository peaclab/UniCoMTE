...
class ClfData:
    ...
    @staticmethod
    def wrap_df_x(x_train, feature_size, index_offset=0):
        """
        ...
        Args:
            ...
            feature_size (int): number of features for a sample.
            index_offset (int): starting offset for the 'index' level of the MultiIndex.
        ...
        # Creating a MultiIndex with two levels: 'index' and 'timestamp'
        index_num = np.repeat(np.arange(timeseries.shape[0]) + index_offset, 4096)
        timestamps = np.tile(np.arange(4096), timeseries.shape[0])

        # create index tuples
        index = pd.MultiIndex.from_arrays([index_num, timestamps], names=['index', 'timestamp'])

        # make multi-layered pd dataframe
    ...
    def wrap_df_y(y_train):
        ...
        Input:
            ...
            - For the ECG implementaiton, it should be a Nx6 one-hot encoded array
    ...
    def wrap_df_test_point(test_point):
        ...
        if test_point.shape[0] == 1:
            np_array_2d = np.squeeze(test_point, axis=0)  # Squeeze only if the first axis is size 1
        else:
            np_array_2d = test_point
